defmodule Elex.Units.FormulaTest do
  ...
  describe "parse/1" do
    ...
    test "parses a symbol that starts with a degree sign" do
      ...
      assert Formula.parse("°C^2") == {:ok, %{"°C" => 2}}
    end

    test "does not fold a trailing degree sign into the previous symbol" do
      assert Formula.parse("C°") == {:ok, %{"C" => 1, "°" => 1}}
      assert Formula.parse("C °") == Formula.parse("C°")
    end
